...
from Lab2.main import agents_count
...
class HealthyState(State):
    ...
    def update(self):
        ...
        for other in self.agent.simulation.agents:
            if isinstance(other.state, ZombieState) and other.state.distance_to(self.agent) < self.agent.simulation.infection_radius:
...
class InfectedState(State):
    def __init__(self, agent, incubation_period):
        ...
        self.agent.speed = self.agent.base_speed * 0.9  # скорость снижена на 10%
...
class ZombieState(State):
    def __init__(self, agent, view_radius=15, view_angle=90):
        super().__init__(agent)
        self.agent.speed = self.agent.base_speed * 0.85  # Зомби движется медленнее
        self.view_radius = view_radius  # Радиус видимости зомби
        self.view_angle = math.radians(view_angle)  # Угол обзора зомби в радианах

    def update(self):
        # Шанс выздороветь с вероятностью 1% на каждой итерации
...
class RecoveredState(State):
    ...
    def update(self):
        ...
        for other in self.agent.simulation.agents:
            if isinstance(other.state, ZombieState) and other.state.distance_to(self.agent) < self.agent.simulation.infection_radius:
                if random.random() < 0.25:  # Вероятность заражения
                    self.agent.state = ZombieState(self.agent)
                    break
...
class Simulation:
    ...
    def is_finished(self):
        return all(isinstance(agent.state, ZombieState) for agent in self.agents) or self.time >= self.T
...
def run_experiments():
    ...
    # Список для хранения результатов (среднее время для каждой конфигурации)
    results = []

    # Пробежимся по всем комбинациям n и m (по порядку)
    for i in range(len(n_values)):
        ...
        print(f"Эксперименты завершены для n={n}, m={m}. Среднее время: {avg_time:.2f} секунд\n")

    # Выводим таблицу результатов
    print("\nТаблица результатов:")
    print("n\tm\tСреднее время (секунды)")
